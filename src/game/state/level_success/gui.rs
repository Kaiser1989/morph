...
pub fn create(config: &Config) -> GuiBuilder<LevelSuccessEvent> {
    GuiBuilder::new("success").size(Value::Auto, Value::Auto).texture(TEX_GUI_FADE, 0).vertical().children(vec![
