...
pub struct LevelFailureState {
    config: Config,
    gui: Gui<LevelFailureEvent>,
    events: Events<LevelFailureEvent>,
    reader: ReaderId<LevelFailureEvent>,
}
...
impl LevelFailureState {
    pub fn new(config: &Config) -> LevelFailureState {
        let config = config.clone();
        let gui = Gui::new(&config);
        let mut events = Events::new();
        let reader = events.register();
        LevelFailureState { config, gui, events, reader }
    }
}

impl GameState for LevelFailureState {
    ...
    fn handle_input(&mut self, input: &InputContext) {
        // handle back button
        if input.back() {
            self.events.write(LevelFailureEvent::Quit);
        }
